...
sms.send = function(phone, message, options, success, failure) {
  ...
  if (typeof options === "string") {
    ...
    window.console.warn(
      "[DEPRECATED] Passing a string as a third argument is deprecated. Please refer to the documentation to pass the right parameter: https://github.com/cordova-sms/cordova-sms-plugin."
    );
...
sms.hasPermission = function(success, failure) {
  // fire
  exec(success, failure, "Sms", "has_permission", []);
};

sms.requestPermission = function(success, failure) {
  // fire
  exec(success, failure, "Sms", "request_permission", []);
};
